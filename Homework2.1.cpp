...
//4 3 2 1
void Sort(std::vector<std::pair<int , int>> PairOfIndexAndValue) {
  std::sort(PairOfIndexAndValue.begin() , PairOfIndexAndValue.end());
}
...
int MinSwapCount(std::vector<int>& vec) {
  ...
  sort(ElementAndIndex.begin() , ElementAndIndex.end());
